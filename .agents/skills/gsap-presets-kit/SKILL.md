...
| ---------- | ----------------------------- | ------- | -------------------------------------------------------- |
| `scroll`   | `boolean | ScrollTriggerOptions` | `true`  | `true` fires on entering viewport (`start: "top 85%", once: true`). `false` plays immediately. Object for full control. |
...
animateFrom(".a", { timeline: tl, scroll: false });
animateFrom(".b", { timeline: tl, scroll: false, position: "-=0.2" });
```
